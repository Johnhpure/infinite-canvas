...
const nodeTypeSchema = z.enum(["image", "text", "config"]);
const generationModeSchema = z.enum(["text", "image"]);
...
export const toolDescriptions: Record<ToolName, string> = {
    ...
    canvas_create_node: "创建任意类型节点：text、image、config。适合创建占位图、配置节点或自定义 metadata 节点。",
    canvas_create_text_node: "在当前画布创建单个文本节点。",
    canvas_create_text_nodes: "批量创建文本节点，适合生成标题、段落、脚本、说明等内容块。",
    canvas_create_config_node: "创建生成配置节点，可指定 text/image 模式和生成参数，可选择立即触发生成。",
    canvas_create_image_prompt_flow: "创建提示词文本节点和图片生成配置节点，并自动连线，可选择立即触发生图。",
    canvas_create_generation_flow: "创建通用生成流程：提示词文本节点、生成配置节点、参考节点连线，可用于文案或生图。",
    ...
    canvas_run_generation: "触发指定节点生成，通常用于配置节点或文本/图片节点。",
};
